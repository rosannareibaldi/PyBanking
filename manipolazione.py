...
df_operazioni.columns = ['id','data_operazione','operazione','variazione','saldo']
...
df_utenti_operazioni['anno_nascita']=pd.to_datetime(df_utenti_operazioni['data_nascita']).dt.year
df_utenti_operazioni['anno_operazioni']=pd.to_datetime(df_utenti_operazioni['data_operazione']).dt.year
df_utenti_operazioni['mese_operazioni']=pd.to_datetime(df_utenti_operazioni['data_operazione']).dt.month
df_utenti_operazioni.drop('saldo_x',axis=1, inplace=True)

#creo un file csv per effettuare il passaggio a Spark 
df_utenti_operazioni.to_csv('lavoro_gruppo.csv', sep=',',header=True)
