...
utente1 = banca.Utente("Lucia", "Ferrari", data_nascita1, "via blabla","Bologna", 3323458192,770,[[0, datetime.datetime(2023, 5, 20, 9, 13, 46, 63466), 'creazione conto', 0, 0],\
...
utente2 = banca.Utente("Rosanna", "Reibaldi", data_nascita2, "via rosa","Foggia", 37507872910, 2200, [[0, data_creazione2, "creazione conto", 2500, 2500],[0, data_prelievo1, "prelievo", - 300, 2200]])
...
utente3 = banca.Utente("Leonardo", "Brunetti", data_nascita3, "via tal dei tali","Perugia", 3240432570, 5000, [[0, data_creazione3, "creazione conto", 5000, 5000]])
...
# Genero 100 province
lista_province = creazione.generaProvince(100)
...
for i in range(100):
    ...
    utente = banca.Utente(lista_nomi[i],lista_cognomi[i],lista_date_nascita[i],lista_indirizzi[i],lista_province[i],lista_num_telefono[i],saldo_attuale,registro)
