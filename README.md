...
Costruita una sessione spark definita ''LavoroFinale'' e caricate le librerie utili all'analisi, sono stati importati file csv precedentemente creato e trasformato contestualmente in un dataframe spark. Le variabili del df sono state poi tipizzate correttamente per agevolare l'analisi.
...
Riconvertito il df in pandas si è proseguito ad un'ulteriore manipolazione dello stesso volta all'esplorazione grafica delle variabili in seaborn e matplotlib.pyplot. Si è arricchita l'analisi con un approccio geospaziale tramite geopandas. 
